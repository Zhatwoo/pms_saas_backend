-- Align public.daily_opening with branch-wide opening sessions (one row per branch per calendar day).
-- Earlier migration 20260430120000_daily_opening.sql used UNIQUE (employee_id, branch_id, opening_date),
-- but PostgREST upserts use ON CONFLICT (branch_id, opening_date) — PostgreSQL requires a matching unique index.
