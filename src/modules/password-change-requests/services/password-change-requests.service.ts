...
export class PasswordChangeRequestsService {
	...
	private async loadUsersByIds(userIds: string[]): Promise<Map<string, UserRow>> {
	...
	private async resolveAuthUserId(
		...
		const email = (record.requesterEmail ?? requester?.email ?? '').trim().toLowerCase();
		if (email) {
	...
	async create(
		...
		const hasPending = records.some(
			(record) => record.requesterUserId === requester.id && record.status === 'pending',
		);
	...
	async findPendingForReviewer(user: AuthenticatedUserProfile) {
		...
		const records = this.buildRecords(logs)
			...
			.filter((record) =>
				user.role === Role.ADMIN ? record.requesterBranchId === user.branchId : true,
			);
	...
	async review(
		...
		if (reviewer.role === Role.ADMIN && existing.requesterBranchId !== reviewer.branchId) {
			throw new ForbiddenException('You can only review requests from your branch');
		}
